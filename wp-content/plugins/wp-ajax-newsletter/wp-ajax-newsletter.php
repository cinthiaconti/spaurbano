...
class ajaxNewsletter {
	...
	function subscribe($email){
		...
		if(!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$", $email)){
			...
			$returnVal['message']="E-mail inválido.";
			return $returnVal;
		}
			...
			if($state == ""){ //new email
				...
				if(ajaxNewsletter::sendConfEmail($email, $confKey)){
					...
					if(ajaxNewsletter::addMember($email, $confKey)){
						...
						$returnVal['message']="Foi enviada uma confirmação para o seu email.";
						return $returnVal;
					}
				}
				
				$returnVal['result']=false;
				$returnVal['message']="Ocorreu um erro. Tente mais tarde.";
				return $returnVal;
				...
				if(ajaxNewsletter::resendConfEmail($email)){
					$returnVal['result']=true;
					$returnVal['message']="A confirmação foi reenviada ao seu email.";
				}else{
					$returnVal['result']=false;
					$returnVal['message']="Ocorrei um erro. Tente mais tarde.";
				}
				return $returnVal;
			}
		}else{ //active email requested a subscription
			$returnVal['result']=false;
			$returnVal['message']="Este email já está inscrito.";
			return $returnVal;
		}
	}
...
function ActivateConfirm(email){
  ...
  return confirm(message);
	...
	function sendEmail($from,$to,$cc,$bcc,$subject,$content){
		...
		@$value = wp_mail($to, $subject, $content, $headers);
	...
	function sendConfEmail($email, $confKey){
		$from = get_option("snews_from");
		$subject = "[Confirmação] Contisaude Newsletter";
		...
		$message = "";
		$message .= "Newsletter da Contisaude - Estética & Spa Urbano\n";
		$message .= "\nPara confirmar sua inscrição, clique no link abaixo:\n";
		$message .= "$confirmationURL\n";
		$message .= "\nCaso não queira receber nosso conteúdo promocional, por favor ignore este email.\n";
...
/**
	...
	function sendSubSuccess($email,$key){
		$from = get_option("snews_from");
		$subject = "[Newsletter Contisaude] " .get_option("snews_subject");
		$title = get_bloginfo("name");
		$url = get_bloginfo("wpurl");
		
		$confirmationURL = $url . "/wp-content/plugins/wp-ajax-newsletter/confirmation.php?del=$key";
		
		$message .= "Você se inscreveu com sucesso na newsletter da $title at:\n$url\n\n\n";

		$message .= "Se você não deseja mais receber nossa newsletter, clique abaixo para cancelar sua inscrição:\n";
		$message .= "  " . $confirmationURL."\n";
		$message = wordwrap($message, 75, "\n");
...
?>
	<form action="javascript:StartFade('<?= $action ?>','ajaxNewsletter','newsletterLoading');"
		...
		<div class="rightAlign" id="newsletterFormDiv" style="text-align: left;padding-left:40px">
			<input
				...
				value="<?php if($email != "") echo $email; else echo 'Digite seu e-mail'; ?>" />
			<input type="hidden" id="newsletter" name="newsletter" value="true" />
			<input class="submit" type="submit" name="newsletterSub" value="Enviar" />
                        <img src="<?= $newsletterURL ?>/loading.gif" alt="Enviando..." title="Enviando..." style="display:none" id="newsletterLoading"/>
		</div>
	</form>
<?php	
	}
	...
	function process_newsletterSub() {
		...
		ajaxNewsletter::printMessageFO("Digite seu e-mail.", false);
		ajaxNewsletter::subscriptionForm();
		exit();//prevent further processing from wordpress
	}
